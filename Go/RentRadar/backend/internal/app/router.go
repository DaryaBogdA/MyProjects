...
func NewRouter(db *sql.DB, projectRoot string) *http.ServeMux {
	...
	mux.HandleFunc("DELETE /api/messages/{id}", middleware.AuthMiddleware(db, messageHandler.DeleteConversation))
	...
	mux.HandleFunc("DELETE /api/bookings/{id}", middleware.AuthMiddleware(db, listingHandler.DeleteBooking))
	...
	mux.HandleFunc("GET /stats.html", serveTemplate(filepath.Join(templatesDir, "stats.html")))
