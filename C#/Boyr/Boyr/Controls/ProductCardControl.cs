...
namespace Boyr.Controls
{
    public partial class ProductCardControl : UserControl
    {
        ...
        private void LoadProduct()
        {
            ...
            lblMetal.Text = $"Металл: {_product.Metal} {_product.Purity}";
            lblWeight.Text = $"Вес: {_product.Weight} г";
            lblGemstone.Text = $"Камень: {_product.Gemstone}";
            if (!string.IsNullOrEmpty(_product.GemCharacteristics))
                lblGemCharacteristics.Text = _product.GemCharacteristics;
